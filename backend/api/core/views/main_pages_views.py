...
def profile(request):
    ...
    return render(request, 'home.html', {'profile': profile[0],'users':users, 'form': EditBioForm(instance=profile[0])})
...
def guest(request,user_id):
    ...
    users=User.objects.all()
    return render(request,'home.html',{'user':user,'profile':profile,'users':users})
